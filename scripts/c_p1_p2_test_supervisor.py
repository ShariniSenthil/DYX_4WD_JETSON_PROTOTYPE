...
"""Runtime C -> P1 -> P2 test-path supervisor.
...
node takes the first two exact local marking coordinates and injects a
...
class CP1P2TestSupervisor(Node):
    """Inject and verify the temporary two-marking runtime path."""
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        super().__init__("c_p1_p2_test_supervisor")
        ...
        self.point_metrics: dict[int, dict[str, Any]] = {
            0: self._new_point_metrics(),
            1: self._new_point_metrics(),
        }
        ...
        self.get_logger().warn(
            "===== PRODUCTION C->P1->P2 TEST SUPERVISOR STARTED ====="
        )
        self.get_logger().warn(
    ...
    def _mission_status_callback(self, message: String) -> None:
        ...
        if state == "ERROR" and not self.result_written:
            ...
            if (
                ...
                self.get_logger().warn(
                    ...
                    "coherent two-point test mission: "
                    f"{error_message}"
                )
                return
        ...
        if (
            ...
            and int(payload.get("total_points", 0)) == 2
            and int(
        ...
        if (
            ...
            and set(self.completed_points) == {0, 1}
        ):
    ...
    def _point_event_callback(self, message: String) -> None:
        ...
        if point_index == 1 and not self.result_written:
            self._finish_from_completed_points()
    ...
    def _build_test_path(self) -> None:
        ...
        self.p1 = self.source_waypoints[0]
        self.p2 = self.source_waypoints[1]
        ...
        c_to_p1_distance = math.hypot(
            self.p1[0] - self.test_start[0],
            self.p1[1] - self.test_start[1],
        )
        p1_to_p2_distance = math.hypot(
            self.p2[0] - self.p1[0],
            self.p2[1] - self.p1[1],
        )
        ...
        self._append_event(
            "TEST_PATH_BUILT",
            {
                ...
                "p1": self.p1,
                "p2": self.p2,
                "c_to_p1_distance_m": c_to_p1_distance,
                "p1_to_p2_distance_m": p1_to_p2_distance,
        ...
        self.get_logger().warn(
            "===== TEMPORARY TEST PATH BUILT ====="
        )
        self.get_logger().warn(
            f"C->P1 distance       : {c_to_p1_distance:.3f} m"
        )
        self.get_logger().warn(
            f"P1->P2 distance      : {p1_to_p2_distance:.3f} m"
        )
        self.get_logger().warn(
    ...
    def _write_ready_file(self) -> None:
        ...
        payload = {
            ...
            "c_to_p1_distance_m": math.hypot(
                self.p1[0] - self.test_start[0],
                self.p1[1] - self.test_start[1],
            ),
            "p1_to_p2_distance_m": math.hypot(
                self.p2[0] - self.p1[0],
                self.p2[1] - self.p1[1],
            ),
            "navigation_point_count": self.expected_navigation_count,
            "maximum_generated_spacing_m": (
                self.maximum_generated_spacing_m
            ),
            "waypoint_radius_m": self.waypoint_radius_m,
            "hold_required_sec": self.hold_required_sec,
    ...
    def _finish_from_completed_points(self) -> None:
        ...
        p1_metrics = self.point_metrics[0]
        p2_metrics = self.point_metrics[1]

        both_completed = (
            bool(p1_metrics["completed"])
            and bool(p2_metrics["completed"])
        )
        position_hold_pass = (
            bool(p1_metrics["passed_position_hold"])
            and bool(p2_metrics["passed_position_hold"])
        )
        heading_pass = (
            bool(p1_metrics["passed_heading"])
            and bool(p2_metrics["passed_heading"])
        )

        if both_completed and position_hold_pass and heading_pass:
            result = "PASS"
            message = (
                "P1 and P2 completed the circular 30 mm waypoint / "
                "3-second hold and heading checks"
            )
        elif both_completed and position_hold_pass:
            result = "FAIL_HEADING_ALIGNMENT"
            message = (
                "P1 and P2 marking holds completed, but master-antenna "
                "heading exceeded the test tolerance"
            )
        else:
            result = "FAIL_MARKING_VERIFICATION"
            message = (
                "P1/P2 completion or continuous position-hold "
                "verification failed"
            )
    ...
    def _write_result(
        ...
        *,
        ...
        payload = {
            ...
            "p1": self.point_metrics[0],
            "p2": self.point_metrics[1],
        ...
        if self.phase == "WAIT_SOURCE":
            if self._phase_age_sec() > self.SOURCE_TIMEOUT_SEC:
                self._write_result(
                    ...
                    message=(
                        "Timed out waiting for prepared first two "
            ...
            if (
                ...
                and len(self.source_waypoints) >= 2
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Inject and verify a temporary production C->P1->P2 "
            "50 mm test path"
        )
    )
    parser.add_argument(
