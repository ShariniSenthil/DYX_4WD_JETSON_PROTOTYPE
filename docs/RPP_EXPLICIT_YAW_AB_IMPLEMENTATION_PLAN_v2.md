# A/B RPP Explicit Yaw — Production Implementation Plan
...
**Firmware branch HEAD verified:** `7f27e7a1d50ea6a1d8a1b4e84ac0eafe6f18a9ec`  
**Full explicit-yaw authority commit:** `0ad2fc56fa9c86f03bbba3a0b3e0fcbbc88036a6`  
**Firmware yaw-speed-limit follow-up:** `7f27e7a1d50ea6a1d8a1b4e84ac0eafe6f18a9ec`  
**Document status:** **PLAN ONLY — NO RPP / BRIDGE IMPLEMENTATION APPLIED**  
**Date:** 2026-09-10  
**Revision:** v2 — firmware re-baselined after `0ad2fc56` / `7f27e7a1`; topic-name disambiguation added
...
the A-side rollback path. The current reviewed firmware source now accepts finite
`trajectory_setpoint.yaw` as heading authority at **all translational speeds**; B-side field use still
requires that firmware to be flashed on the rover and independently bench-verified before motion.
...
The firmware dependency has advanced beyond the original stationary-only baseline.

Historical sequence verified on `fix/rover-land-state`:

```text
67b4e5c
    → stationary absolute-yaw pivot support only

0ad2fc56fa9c86f03bbba3a0b3e0fcbbc88036a6
    → rover: enable absolute yaw steering at any speed
    → implements the full four-case heading-authority contract required by this plan

7f27e7a1d50ea6a1d8a1b4e84ac0eafe6f18a9ec
    → current verified branch HEAD
    → parent = 0ad2fc56...
    → adds optional OFFBOARD `trajectory_setpoint.yawspeed` handling as a yaw-rate limit
```

At current firmware HEAD `7f27e7a1`, `DifferentialVelControl` resolves the command as:

```text
finite yaw + moving velocity
    → speed = velocity magnitude
    → bearing = explicit yaw

finite yaw + zero/near-zero velocity
    → speed = 0
    → bearing = explicit yaw

invalid/NaN yaw + moving velocity
    → speed = velocity magnitude
    → bearing = velocity-vector bearing

invalid/NaN yaw + zero/near-zero velocity
    → speed = 0
    → bearing = current vehicle yaw
```

Therefore the **firmware source-implementation gate is resolved**. The B-side is no longer blocked on
writing moving-yaw firmware support.

The remaining firmware gate is operational verification:

```text
required firmware commit present in the image
    → firmware builds
    → image flashed to the rover
    → bench/SITL proof confirms explicit-yaw precedence and fallback behavior
    → only then allow B-side field motion
```

### `7f27e7a1` yaw-speed-limit interaction

`7f27e7a1` also makes finite `trajectory_setpoint.yawspeed` an OFFBOARD yaw-rate **limit** in
`DifferentialAttControl`. This plan still does **not** enable that feature.

The proposed B bridge mask remains:

```text
2503 = velocity + yaw, with IGNORE_YAW_RATE set
```

In the reviewed PX4 MAVLink receiver, `POSITION_TARGET_TYPEMASK_YAW_RATE_IGNORE` explicitly maps the
received yaw-rate field to:

```text
trajectory_setpoint.yawspeed = NaN
```

Therefore the `7f27e7a1` optional yawspeed limiter is not selected by the planned B contract even
though the ROS `PositionTarget.yaw_rate` storage value is `0.0`. The type mask remains the authority.

Bench evidence must still confirm `trajectory_setpoint.yawspeed` is non-finite under mask `2503` so
this interaction is proven on the actual built firmware.

The ROS2 A-side remains backward-compatible because Mode A continues to send `IGNORE_YAW`, causing
PX4 to use the existing vector-bearing fallback while moving and current-yaw hold at rest.
...
| 8 | B-side straight tracking field test | **Not performed — firmware source gate resolved; flash + bench proof required first** |
| 9 | B-side pivot/transition field test | **Not performed — firmware source gate resolved; flash + bench proof required first** |
...
15. No ROS-side yaw-rate feed-forward or yawspeed-limit command is introduced; Mode B keeps
    `IGNORE_YAW_RATE`. The firmware branch contains optional yawspeed-limit support, but this migration
    deliberately leaves it inactive.
...
22. PX4 source support for explicit yaw at all speeds is verified at `0ad2fc56` and retained at
    current HEAD `7f27e7a1`; full B-side field activation is still blocked until that firmware is
    built/flashed and the precedence/fallback contract is bench-verified.
...
The existing RPP already publishes:

```text
/rpp/command_speed_mps
```

That topic is a `Float64` command-speed telemetry/output value and is **not** the proposed atomic
velocity+yaw authority topic. During implementation, field logging, `ros2 topic echo`, and grep work,
do not confuse:

```text
/rpp/command_speed_mps    # existing scalar speed topic
/rpp/command              # proposed atomic B-side control command
```

The names do not collide at ROS graph level. This note is retained to prevent operator/debugging
confusion; the first implementation does not rename either topic.
...
# 17. Firmware contract required by the B-side — source implementation verified
...
**Current verification:** commit `0ad2fc56fa9c86f03bbba3a0b3e0fcbbc88036a6` implements this
full moving+stationary explicit-yaw precedence, and current branch HEAD
`7f27e7a1d50ea6a1d8a1b4e84ac0eafe6f18a9ec` retains the same `DifferentialVelControl` behavior.
The implementation requirement is therefore satisfied in source; build/flash/bench proof remains an
acceptance gate.
...
## 17.2.1 Current firmware source status

```text
0ad2fc56  → full explicit-yaw heading authority at any speed
7f27e7a1  → retains that authority and adds optional finite-yawspeed limiting
```
...
```text
use explicit yaw
ignore yaw-rate / yawspeed
```

The bridge mask `2503` keeps `POSITION_TARGET_TYPEMASK_YAW_RATE_IGNORE` set. PX4's MAVLink receiver
converts that ignored yaw-rate field to `trajectory_setpoint.yawspeed = NaN`, so the optional
`7f27e7a1` limiter is not part of the B contract.

Do not remove the ignore bit merely because the firmware can now consume yawspeed. That would be a
separate controller feature and a separate A/B hypothesis.
...
Stationary-only firmware `67b4e5c` remains an invalid B configuration:
...
The source-level moving-yaw gap was closed by `0ad2fc56` and that behavior remains present at current
verified firmware HEAD `7f27e7a1`.

A B test is still invalid if the rover is actually running an older image or if the current firmware
was not bench-proven after build/flash.

Required pre-field evidence:

```text
finite moving yaw overrides vector bearing
finite stationary yaw commands pivot/hold
invalid moving yaw falls back to vector bearing
invalid stationary yaw holds current yaw
IGNORE_YAW_RATE produces non-finite trajectory_setpoint.yawspeed
```
...
Firmware source gate:

```text
67b4e5c   = stationary-only historical baseline; not valid for B TRACK
0ad2fc56  = full explicit-yaw heading authority at any speed
7f27e7a1  = current verified branch HEAD; retains 0ad2 behavior
```

The source implementation gate is **resolved**.

The remaining gate before B field activation is:

```text
build → flash → bench/SITL verify four-case heading truth table
      → verify IGNORE_YAW_RATE yields trajectory_setpoint.yawspeed = NaN
```
...
No field B activation yet until the current firmware image is built/flashed and the bench gate is closed.
...
Use the current production firmware or current full-yaw firmware; either should preserve the A-side
because yaw remains ignored. If using `7f27e7a1`, verify the A-side still yields non-finite yaw and
yawspeed at `trajectory_setpoint`.
...
```text
firmware containing 0ad2fc56 behavior (current verified branch HEAD: 7f27e7a1)
built and flashed to the rover
four-case explicit-yaw precedence/fallback bench test passed
IGNORE_YAW_RATE → trajectory_setpoint.yawspeed is non-finite verified
```
...
- [x] source at `0ad2fc56` implements finite moving yaw precedence over vector bearing.
- [x] source at current verified HEAD `7f27e7a1` retains finite stationary yaw pivot/hold.
- [x] source retains moving invalid-yaw fallback to vector bearing.
- [x] source retains stationary invalid-yaw current-heading hold.
- [x] PX4 MAVLink receiver maps `IGNORE_YAW_RATE` to `trajectory_setpoint.yawspeed = NaN`.
- [ ] the intended firmware image has been built and flashed to the rover.
- [ ] bench/SITL evidence confirms all four heading-authority cases on that image.
- [ ] bench evidence confirms B mask `2503` produces non-finite `trajectory_setpoint.yawspeed`.
- [ ] ROS B-side still does not activate yaw-rate/yawspeed control.
...
21. Is the full moving-yaw firmware source (`0ad2fc56` behavior, retained at `7f27e7a1`) actually
    built/flashed and bench-verified before B field activation?
22. Are yaw gains/lookahead/speeds unchanged in this interface migration?
23. Does B keep `IGNORE_YAW_RATE`, and does PX4 evidence show non-finite `trajectory_setpoint.yawspeed`
    so the `7f27e7a1` optional yawspeed limiter stays inactive?
...
```text
ROS yaw-rate / yawspeed command activation
RO_YAW_* tuning
RD_* tuning
...
9. Build + flash current firmware containing the verified `0ad2fc56` full-yaw behavior
10. Bench-prove firmware precedence/fallback and ignored-yawspeed behavior
...
# 38. Independent verification record

An independent review pass checked this document's falsifiable claims directly against the live
repositories rather than against the document's own assertions. Findings below; no RPP/bridge code
was changed by this review — plan and doc edits only.

## 38.1 v1 review (2026-09-10)

Verified accurate against source:

```text
git HEAD/tree/RPP-blob/bridge-blob hashes — exact match via git rev-parse / git ls-tree
repository name and branch — exact match via git remote -v / git branch --show-current
TYPE_MASK_VELOCITY_ONLY = 3527 bit math — matches cmd_vel_bridge.py:52-60 exactly
ENU field mapping (velocity.x=East, velocity.y=North, yaw=0.0, yaw_rate=0.0)
    — matches cmd_vel_bridge.py:297-309 exactly
bearing convention (north=speed*sin(bearing), east=speed*cos(bearing))
    — confirmed at multiple rpp_controller_node.py call sites
publish_stop() → publish_velocity_ned(0.0, 0.0) — confirmed at rpp_controller_node.py:7036-7039;
    validates the §3.3 "generic zero publisher has no hard-safety/HOLD distinction" finding as real
NATIVE_CARRIER / "±60deg carrier" language — confirmed in legacy_alignment.py:41 and
    rpp_controller_node.py:4788/9772, not invented
rpp_interfaces package does not yet exist — no RppCommand/rpp_explicit_yaw string anywhere in src/,
    so the "NEW" file classifications in §6 are accurate and non-conflicting
proposed rpp_interfaces/CMakeLists.txt skeleton (§7.4) matches the one existing in-repo precedent
    (mission_manager_interfaces/CMakeLists.txt) with the correct added std_msgs dependency
```

One HIGH finding: the firmware "reviewed commit" `67b4e5c` (stationary-only) had already been
superseded by `0ad2fc56` ("rover: enable absolute yaw steering at any speed"), committed roughly one
hour after `67b4e5c` and implementing the exact four-case moving+stationary heading-authority
contract the plan lists as its own B-side blocking requirement. One MEDIUM finding: `/rpp/command`
(proposed) vs. the existing unrelated `/rpp/command_speed_mps` is easily confused in logs/greps.

## 38.2 v2 review (2026-09-10)

v2 was written to resolve exactly those two findings. Re-verified independently rather than taken on
trust:

```text
0ad2fc56fa9c86f03bbba3a0b3e0fcbbc88036a6 (DifferentialVelControl.cpp)
    — read the full diff: implements the plan's required §17.1 truth table exactly (reordered
      branches, behaviorally identical); touches only DifferentialVelControl.cpp, so the change is
      isolated to heading-authority resolution as claimed

7f27e7a1d50ea6a1d8a1b4e84ac0eafe6f18a9ec (DifferentialAttControl.cpp)
    — read the full diff: adds an OPTIONAL yaw-rate limit gated on
      flag_control_offboard_enabled AND finite(trajectory_setpoint.yaw) AND
      finite(trajectory_setpoint.yawspeed); when either is non-finite, the limiter stays at the
      existing RO_YAW_RATE_LIM ceiling — no behavior change from current production for either A or B
    — confirmed 0ad2fc56 is an ancestor of 7f27e7a1, and 7f27e7a1 does not touch
      DifferentialVelControl.cpp, so the §17.1 heading-authority logic v2 depends on is retained
      unmodified

mavlink_receiver.cpp:1108-1109 (SET_POSITION_TARGET_LOCAL_NED handler)
    setpoint.yaw      = (type_mask & POSITION_TARGET_TYPEMASK_YAW_IGNORE)      ? NAN : target...yaw
    setpoint.yawspeed = (type_mask & POSITION_TARGET_TYPEMASK_YAW_RATE_IGNORE) ? NAN : target...yaw_rate
    — confirms the §17.2.1/§20.6 "IGNORE_YAW_RATE -> trajectory_setpoint.yawspeed = NaN" claim
      directly in source, not just asserted
    — also confirms (not cited by the plan, but relevant) that Mode A's IGNORE_YAW bit likewise
      forces trajectory_setpoint.yaw = NaN, so Mode A is structurally inert to BOTH new firmware
      features automatically, on both the vector-bearing-fallback path and the new yaw-rate limiter

rover_ws git HEAD unchanged at 1fc7bf3 across both review passes
    — confirms the Phase-0 ROS-side baseline in this document is still valid
```

No CRITICAL or HIGH findings on v2. One MEDIUM/process note, not a document defect: the firmware
branch moved three times within about 90 minutes on 2026-09-10 (`67b4e5c` -> `0ad2fc56` ->
`7f27e7a1`). Re-confirmed immediately before this v2 document was finalized that `7f27e7a1` was
still the branch tip (`git fetch` + `git rev-parse origin/fix/rover-land-state`). Given that cadence,
re-check the firmware branch HEAD again immediately before Phase 1 begins rather than trusting this
record indefinitely — the plan's own §37 "STOP and re-verify if source has moved" rule applies here.

## 38.3 Scope not covered by this review

This was a documentation/source-accuracy audit, not a code review of an implementation (none exists
yet — this document is PLAN ONLY) and not a field or bench test. It does not substitute for the
build/flash/bench verification gates the plan itself requires in Phase 0/8/9 before any B-side field
motion.
