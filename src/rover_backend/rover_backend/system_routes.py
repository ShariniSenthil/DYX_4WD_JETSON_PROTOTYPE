...
def collect_network_status() -> dict[str, Any]:
    ...
    is_wifi_interface = bool(
        selected_interface
        and selected_interface.lower().startswith(
            ("wl", "wlan")
        )
    )
    ...
    payload: dict[str, Any] = {
        ...
        "ip_matches_configuration": (
            selected_ip == settings.rover_ip
        ),
        "connected": interface_is_up,
        "wifi_interface": is_wifi_interface,
        "wifi_connected": bool(
            interface_is_up
            and is_wifi_interface
            and wifi_ssid
        ),
        "wifi_ssid": wifi_ssid,
...
def build_mission_status_payload() -> dict[str, Any]:
    ...
    mission = rover_state.section(
        "mission"
    )

    state_name = str(
        mission.get("state") or "EMPTY"
    ).strip().upper()

    return {
        "mission_id": mission.get("mission_id"),
        "filename": mission.get("filename"),
        "checksum_sha256": mission.get(
            "checksum_sha256"
        ),
        "coordinate_mode": mission.get(
            "coordinate_mode"
        ),
        "extension_mode": mission.get(
            "extension_mode"
        ),
        "dummy_point_distance_m": mission.get(
            "dummy_point_distance_m"
        ),
        "row_transition_threshold_m": mission.get(
            "row_transition_threshold_m"
        ),
        "state": state_name,
        "state_lower": state_name.lower(),
        "loaded": bool(
            mission.get("loaded", False)
        ),
        "ready": bool(
            mission.get("ready", False)
        ),
        "total_points": max(
            0,
            _safe_int(
        ...
        "marking_active": bool(
            mission.get("marking_active", False)
        ),
        "pause_reason": mission.get("pause_reason"),
        "resume_available": bool(
            mission.get("resume_available", False)
        ),
        ...
        "spray_controller_ready": bool(
            mission.get("spray_controller_ready", False)
        ),
        ...
        "alignment_active": bool(
            mission.get("alignment_active", False)
        ),
        "hold_elapsed_sec": (
            _finite_float(
                mission.get("hold_elapsed_sec"),
...
def build_telemetry_payload() -> dict[str, Any]:
    ...
    local_x = _finite_float(
        position.get("local_x_m")
    )
    local_y = _finite_float(
        position.get("local_y_m")
    )
    ground_speed = (
        _finite_float(
    ...
    return {
        ...
        "mode": str(
            vehicle.get("mode") or "UNKNOWN"
        ),
        "connected": bool(
            vehicle.get("connected", False)
        ),
        ...
        "rtk_fixed": bool(
            gps.get("rtk_fixed", False)
        ),
        "rtk_healthy": bool(
            rtk.get("healthy", False)
        ),
        "rtk_correction_age_sec": rtk.get(
            "correction_age_sec"
        ),
        "emergency_stop": bool(
            safety.get("emergency_stop", True)
        ),
        "mission_enable": bool(
            safety.get("mission_enable", False)
        ),
        "mission_state": mission["state"],
        "marking_active": mission[
            "marking_active"
        ],
        # Flat additive fields for the current frontend adapter.
        # Signed values remain visible even when they exceed 50 mm.
        "cross_track_error_mm": accuracy.get(
            "cross_track_error_mm"
        ),
        "cross_track_abs_mm": accuracy.get(
            "cross_track_abs_mm"
        ),
        "cross_track_side": accuracy.get(
            "cross_track_side"
        ),
        "front_back_error_mm": accuracy.get(
            "front_back_error_mm"
        ),
        "front_back_abs_mm": accuracy.get(
            "front_back_abs_mm"
        ),
        "front_back_position": accuracy.get(
            "front_back_position"
        ),
        "radial_error_mm": accuracy.get(
            "radial_error_mm"
        ),
        "closest_radial_error_mm": accuracy.get(
            "closest_radial_error_mm"
        ),
        "accuracy_status": accuracy.get(
            "accuracy_status"
        ),
        "accuracy_pass": bool(
            accuracy.get("accuracy_pass", False)
        ),
...
def _require_ros_bridge() -> None:
    if not ros_bridge.running:
        raise HTTPException(
            ...
            detail=(
                "The rover ROS bridge is not running."
            ),
        )
...
async def healthz(
    ...
    backend_online = bool(
        backend.get("online", False)
    )
    ros_node_started = bool(
        ros.get("node_started", False)
    )
    ros_connected = bool(
        ros.get("connected", False)
    )
    ...
    safe_non_driving = bool(
        safety.get("emergency_stop", True)
        and not safety.get("mission_enable", False)
    )
    ...
    return {
        ...
        "backend_started_at": backend.get(
            "started_at"
        ),
        "backend_uptime_sec": _iso_age_seconds(
            backend.get("started_at")
        ),
        "ros_node_started": ros_node_started,
        "ros_connected": ros_connected,
        "ros_last_message_age_sec": (
            _iso_age_seconds(
                ros.get("last_message_at")
            )
        ),
        "backend_heartbeat_last_age_sec": (
            _iso_age_seconds(
                ros.get("last_heartbeat_at")
            )
        ),
        "backend_heartbeat_healthy": bool(
            safety.get(
...
async def emergency_stop(
    ...
    try:
        safety = await run_in_threadpool(
            ros_bridge.force_emergency_stop
        )
...
async def release_emergency_stop(
    ...
    try:
        safety = await run_in_threadpool(
            ros_bridge.release_emergency_stop
        )
    except RuntimeError as error:
        raise HTTPException(
            status_code=(
    ...
    return {
        ...
        "message": (
            ...
            "mission movement remains disabled."
        ),
        "safety": safety,
    }