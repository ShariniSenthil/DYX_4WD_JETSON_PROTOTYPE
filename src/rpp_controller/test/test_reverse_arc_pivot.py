...
def fly(turn_deg, true_ahead, true_left, cfg=None, *, forward_only=False,
    ...
    limit; ~20 ms transport. Returns final nozzle cross/along to the new line."""
...
def test_timeout_and_reverse_cap_abort_to_fallback():
    ...
    arc = ReverseArcPivot(ReverseArcConfig(max_reverse_travel_m=0.05))
    arc.start(0.0, 0.0, math.radians(90), 0, 0, math.radians(90))
...
def test_large_turns_end_within_60mm_of_the_new_line(turn):
...
def test_small_turns_end_within_110mm_across_centre_spread(turn):
    for ahead, left in (LEFT_TRUE if turn > 0 else RIGHT_TRUE):
        result = fly(turn, ahead, left)
...
def test_calibrated_centre_beats_the_stationary_pivot_by_far(turn):
...
@pytest.mark.parametrize("turn", [90, -90])
def test_forward_only_px4_is_detected_and_falls_back(turn):
    ahead, left = (0.49, 0.03) if turn > 0 else (0.39, -0.044)
    result = fly(turn, ahead, left, forward_only=True)
...
def _node(enabled=True, explicit=True):
    ...
    node = NS(
        ...
        reverse_arc=ReverseArcPivot(ReverseArcConfig()),
