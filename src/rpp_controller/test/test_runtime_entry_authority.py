...
These tests match the production architecture on feat/fixed-survey-trajectory:

* surveyed P1..Pn are prepared separately in the fixed map frame;
* START-time C comes from current MAVROS local odometry;
* a temporary C->P1 path is generated at <=50 mm spacing;
* a real pivot is followed by a fresh local-odom C' reanchor;
* the fixed prepared /nav_path and its signature are not mutated;
* RPP no longer owns gp_origin/fused-global conversion for C/C'.
"""
...
def test_retained_callbacks_cannot_move_locked_start_c_before_real_pivot_reanchor():
    ...
    # Simulate later odometry/retained DDS activity while the first approach
    # is already locked. The START anchor must remain unchanged.
...
def test_post_pivot_reanchor_uses_fresh_local_odom_c_prime_and_resets_cursor():
    ...
    # Simulate physical pivot walk and the fresh odometry measured after the
    # pivot settle certificate.
    ...
    expected_bearing = math.atan2(0.0 - (-0.06), 6.0 - 0.08)
    assert math.isclose(
        controller.c_line_bearing,
        expected_bearing,
        rel_tol=0.0,
...
def test_obsolete_selector_is_not_control_authority_anymore():
    ...
    # The compatibility helper may remain in runtime_entry.py for old tooling,
    # but production control_loop must not use it.
