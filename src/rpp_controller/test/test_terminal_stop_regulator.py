...
def settle(driver, *, along=0.012, cross=0.016, x_m=0.0):
    result = None
    # Make the helper independent of Driver.dt. The stationary detector needs
    # one full configured position window, so fixed cycle counts are unsafe for
    # tests that intentionally use a faster control timestep.
    required_steps = (
        math.ceil(
            driver.regulator.config.stationary_window_sec / driver.dt
        )
        + 2
    )
