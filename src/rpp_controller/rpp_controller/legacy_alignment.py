...
class LegacyAlignmentConfig:
    ...
    # RPP releases the absolute-yaw target at 4 deg. If heading rebounds to
    # this threshold before the stop certificate completes, re-enter pivot.
    settle_reentry_heading_rad: float = math.radians(6.0)
    ...
    def __post_init__(self) -> None:
        ...
        if not (
            self.native_release_heading_rad
            < self.settle_reentry_heading_rad
            < self.pivot_enter_rad
        ):
            raise ValueError(
                "pivot heading thresholds must satisfy "
                "native_release < settle_reentry < pivot_enter"
            )
...
class LegacyAlignmentLifecycle:
    ...
    def _step_pivot_settle(
        ...
        if heading_abs >= self.config.settle_reentry_heading_rad:
            return self._enter_pre_pivot_stop(
                previous,
                sample,
                "SETTLE_REENTER_PIVOT",
            )
        if not self._chassis_stationary_debounced(sample):
            self.reset_dwell_timers()
            return self._result(
