...
class RPPController(Node):
    ...
    def validate_parameters(self):
        ...
        if not math.isclose(self.cruise_speed, 1.00, abs_tol=1.0e-6):
            raise ValueError(
                "production cruise_speed_mps must be exactly 1.00 m/s"
            )
    ...
    def control_loop(self):
        ...
        #   at 1.00m/s until xtrack <=8mm and heading <=4deg for 0.20s.
        #
        # Guard:
        #   If post-pivot xtrack is already >50mm, leave the short capture phase
        #   and hand control to the global xtrack controller, still at 1.00m/s.
        #
        # Final 500 mm semantic-goal distance deceleration to 0.15 m/s and
        ...
        if self.segment_alignment_active:
            ...
            if abs(alignment_cross_track) > self.segment_fast_capture_max_cross_track:
                # Hardened fallback: if post-pivot xtrack is already too
                # large for the short capture phase, hand ownership to the
                # global 1.00 m/s xtrack recovery controller instead.
                ...
                release_heading_limit = self.terminal_native_pivot_release_error
                ...
                if release_geometry_valid:
                    ...
                    alignment_hold_elapsed = (
                        self.get_clock().now() - self.alignment_inside_since
                    ).nanoseconds / 1e9

                    if alignment_hold_elapsed >= self.alignment_hold_sec:
                        self.segment_alignment_active = False
                        self.segment_alignment_pivot_complete = False
                        self.segment_pivot_keeper_started_at = None
                        self.alignment_inside_since = None
