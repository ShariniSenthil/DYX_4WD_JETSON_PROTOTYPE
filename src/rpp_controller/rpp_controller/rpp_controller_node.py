...
class RPPController(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("legacy_pivot_post_settle_hold_sec", 0.20)
        ...
        self.legacy_alignment = LegacyAlignmentLifecycle(
            LegacyAlignmentConfig(
                ...
                settle_reentry_heading_rad=self.pivot_exit_angle,
        ...
        if self.rpp_explicit_yaw_enabled:
            # Patch 3 carries the exact owning-call-site ENU yaw beside the
            # final N/E velocity. The Patch-2 bridge still rejects every B
            # command, so field actuation remains disabled until Patch 4.
    ...
    def _publish_legacy_native_carrier(
        ...
        if self.rpp_explicit_yaw_enabled:
            ...
            self.log_control(
                ...
                + f" | command_bearing={math.degrees(command_bearing):.1f}deg"
    ...
    def terminal_native_pivot_command(
        ...
        if self.terminal_native_pivot_active:
            ...
            # completed native pivot is allowed to re-enter at the configured
            # 6 deg settle hysteresis threshold.
            reentering_from_settle = (
                self.legacy_alignment.phase is LegacyAlignmentPhase.PRE_PIVOT_STOP
                and self.legacy_alignment.native_carrier_issued
            )
            entry_error = (
                self.pivot_exit_angle
                if reentering_from_settle
                else self.terminal_native_pivot_enter_error
            )
            if abs(true_error) < entry_error:
        ...
        # Production B mode is pure absolute yaw. Keep the true
        # segment bearing as the only yaw target; no synthetic +/-60 degree
        # carrier enters the explicit-yaw command path.
        if self.rpp_explicit_yaw_enabled:
            self.terminal_native_pivot_request_bearing = true_bearing
            return True, true_bearing, true_error

        # Legacy A-mode rollback only: preserve the historical velocity-vector
        # carrier used to request PX4 native turning.
        turn_sign = 1.0 if true_error > 0.0 else -1.0
        carrier_error = turn_sign * self.terminal_native_pivot_request_error
        request_bearing = self.normalize_angle(self.current_yaw + carrier_error)
    ...
    def publish_precision_velocity_ned(self, command_bearing, result):
        ...
        if self.rpp_explicit_yaw_enabled:
            ...
            if not self.velocity_pub.publish_with_yaw(
                ...
                self._reset_precision_regulator("EXPLICIT_YAW_COMMAND_REJECTED")
                self._record_published_translational_speed(0.0)
                self.publish_motion_profile_monitor(0.0)
                return 0.0, 0.0, 0.0
        else:
