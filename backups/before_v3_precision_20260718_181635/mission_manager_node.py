...
class MissionManager(Node):
    """
    Sequence the interpolated P1-P4 marking mission.

    P1-only runtime approach:
      - when the mission is enabled for the first time, use the rover's
        fresh local ENU position as the start point;
      - create A1 exactly p1_approach_distance_m before P1 on the
        current-position -> P1 line;
      - publish A1 as one direct pass-through target, with no generated
        points between the current rover position and A1;
      - interpolate only the final A1 -> P1 corridor at
        p1_corridor_spacing_m;
      - preserve the original P1 -> P2 -> P3 -> P4 interpolated path.

    A1 and the A1 -> P1 corridor points are pass-through targets. P1
    remains the first real marking point. P2-P4 retain the existing
    mission behavior.
    """
    ...
    def __init__(self):
        ...
        self.declare_parameter("navigation_tolerance_m", 0.03)
        self.declare_parameter("intermediate_switch_distance_m", 0.10)
        self.declare_parameter("pass_through_lookahead_distance_m", 0.90)
        ...
        self.declare_parameter("p1_approach_distance_m", 1.00)
        self.declare_parameter("p1_corridor_spacing_m", 0.05)
        self.declare_parameter("p1_marking_handoff_distance_m", 0.18)
        self.declare_parameter("p1_a1_switch_distance_m", 0.30)
        self.declare_parameter("p1_corridor_lookahead_distance_m", 0.25)
        ...
        self.p1_approach_distance = float(
            self.get_parameter("p1_approach_distance_m").value
        )
        self.p1_corridor_spacing = float(
            self.get_parameter("p1_corridor_spacing_m").value
        )
        self.p1_marking_handoff_distance = float(
            self.get_parameter("p1_marking_handoff_distance_m").value
        )
        self.p1_a1_switch_distance = float(
            self.get_parameter("p1_a1_switch_distance_m").value
        )
        self.p1_corridor_lookahead_distance = float(
            self.get_parameter(
                "p1_corridor_lookahead_distance_m"
            ).value
        )

        self.validate_parameters()

        # Original source data from trajectory_generator.
        self.pending_path = None
        self.pending_marking_waypoints = None
        self.source_path = []
        self.source_marking_waypoints = []
        self.source_signature = None

        # Runtime path used for control.
        self.path_points = []
        self.marking_flags = []
        self.marking_numbers = []
        self.runtime_signature = None
        self.runtime_start_point = None
        self.p1_approach_point = None
        self.p1_approach_index = None
        self.p1_marking_index = None
        self.runtime_built = False
        self.runtime_build_requested = False
        self.a1_direct_log_emitted = False
        self.p1_corridor_log_emitted = False
        ...
        self.p1_approach_active_pub = self.create_publisher(
            Bool,
            "/p1_approach_active",
            10,
        )
        self.p1_direct_a1_active_pub = self.create_publisher(
            Bool,
            "/p1_direct_a1_active",
            10,
        )
        ...
        self.publish_p1_approach_active(False)
        self.publish_p1_direct_a1_active(False)
        ...
        self.get_logger().warn(
            "===== P1-APPROACH LOOKAHEAD MISSION MANAGER STARTED ====="
        )
        self.get_logger().warn(
            f"P1 approach distance : {self.p1_approach_distance:.2f} m"
        )
        self.get_logger().warn(
            f"P1 corridor spacing  : <= "
            f"{self.p1_corridor_spacing:.3f} m"
        )
        self.get_logger().warn(
            f"P1 marking handoff   : "
            f"{self.p1_marking_handoff_distance:.2f} m"
        )
        self.get_logger().warn(
            f"A1 corridor switch  : "
            f"{self.p1_a1_switch_distance:.2f} m before A1"
        )
        self.get_logger().warn(
            f"P1 corridor lookahead: "
            f"{self.p1_corridor_lookahead_distance:.2f} m"
        )
        self.get_logger().warn(
            "P2-P4 path handling  : existing interpolated path"
        )
    ...
    def validate_parameters(self):
        ...
        positive_values = {
            ...
            "p1_approach_distance_m": self.p1_approach_distance,
            "p1_corridor_spacing_m": self.p1_corridor_spacing,
            "p1_marking_handoff_distance_m": (
                self.p1_marking_handoff_distance
            ),
            "p1_a1_switch_distance_m": self.p1_a1_switch_distance,
            "p1_corridor_lookahead_distance_m": (
                self.p1_corridor_lookahead_distance
            ),
        }
        ...
        if (
            ...
            raise ValueError(
                "p1_marking_handoff_distance_m must be greater than "
                "navigation_tolerance_m"
            )
        if (
            self.p1_marking_handoff_distance
            >= self.p1_approach_distance
        ):
            raise ValueError(
                "p1_marking_handoff_distance_m must be less than "
                "p1_approach_distance_m"
            )
        if self.p1_a1_switch_distance >= self.p1_approach_distance:
            raise ValueError(
                "p1_a1_switch_distance_m must be less than "
                "p1_approach_distance_m"
            )
        if self.p1_a1_switch_distance <= self.intermediate_switch_distance:
            raise ValueError(
                "p1_a1_switch_distance_m must be greater than "
                "intermediate_switch_distance_m"
            )
        if (
            self.p1_corridor_lookahead_distance
            ...
            raise ValueError(
                "p1_corridor_lookahead_distance_m must be less than "
                "p1_approach_distance_m"
            )
    ...
    def path_callback(self, msg):
        ...
        self.try_accept_source_mission()
    ...
    def try_accept_source_mission(self):
        ...
        if new_signature == self.source_signature:
            return
        ...
        self.source_path = list(self.pending_path)
        self.source_marking_waypoints = list(
            self.pending_marking_waypoints
        )
        self.source_signature = new_signature
        ...
        self.p1_marking_index = None
        self.runtime_built = False
        self.runtime_build_requested = self.mission_enabled
        self.a1_direct_log_emitted = False
        self.p1_corridor_log_emitted = False
        self.current_path_index = 0
        self.mission_complete = False
        ...
        self.get_logger().warn(
            "========== SOURCE MISSION ACCEPTED =========="
        )
        self.get_logger().warn(
            f"Original path points : {len(self.source_path)}"
        )
        self.get_logger().warn(
            f"Original markings    : {original_marking_indices}"
        )
        self.get_logger().warn(
            f"Source mission ID    : {new_signature[:12]}"
        )
        self.get_logger().warn(
            "Direct A1 target and A1->P1 corridor will be built on first mission enable"
        )
    ...
    def interpolate_segment(start, end, spacing):
        ...
        if not math.isfinite(length) or length <= 0.0:
            raise ValueError("runtime interpolation segment is invalid")
    ...
    def build_runtime_p1_approach(self):
        if not self.source_path or not self.source_marking_waypoints:
            raise ValueError("source mission is not available")
        if not self.odometry_is_fresh():
            raise ValueError("fresh odometry is not available")

        source_flags, _ = self.match_marking_points(
            self.source_path,
            self.source_marking_waypoints,
        )
        if source_flags is None:
            raise ValueError("source marking indices cannot be resolved")

        source_p1_index = next(
            (
                index
                for index, flag in enumerate(source_flags)
                if flag
            ),
            None,
        )
        if source_p1_index is None:
            raise ValueError("P1 was not found in the source path")

        start = (float(self.current_x), float(self.current_y))
        p1 = self.source_marking_waypoints[0]
        delta_x = p1[0] - start[0]
        delta_y = p1[1] - start[1]
        start_to_p1 = math.hypot(delta_x, delta_y)

        # Keep A1 meaningfully ahead of the rover at mission start.
        # The 0.18 m marking handoff applies near P1 and does not need
        # to be added to the start-distance requirement.
        minimum_start_distance = (
            self.p1_approach_distance
            + self.intermediate_switch_distance
        )
        if start_to_p1 <= minimum_start_distance:
            raise ValueError(
                f"rover is only {start_to_p1:.3f}m from P1; "
                f"required > {minimum_start_distance:.3f}m "
                "to create a direct A1 target before the 1.00m corridor"
            )

        unit_x = delta_x / start_to_p1
        unit_y = delta_y / start_to_p1

        a1 = (
            p1[0] - self.p1_approach_distance * unit_x,
            p1[1] - self.p1_approach_distance * unit_y,
        )

        # A1 is one direct target. There are intentionally no
        # generated points between the rover's mission-start position
        # and A1. Dense interpolation is used only on the final A1 -> P1
        # precision corridor.
        a1_to_p1 = self.interpolate_segment(
            a1,
            p1,
            self.p1_corridor_spacing,
        )

        runtime_path = (
            [a1]
            + a1_to_p1
            + self.source_path[source_p1_index + 1:]
        )

        if len(runtime_path) > self.max_path_points:
            raise ValueError(
                f"runtime path contains {len(runtime_path)} points; "
                f"maximum is {self.max_path_points}"
            )
        ...
        if flags is None:
            raise ValueError(
                "runtime path does not contain all original markings"
            )

        self.path_points = runtime_path
        self.marking_flags = flags
        self.marking_numbers = numbers
        self.runtime_start_point = start
        self.p1_approach_point = a1
        self.p1_approach_index = 0
        self.p1_marking_index = next(
            index
            for index, number in enumerate(numbers)
            if number == 1
        )
        self.runtime_signature = self.make_signature(
            runtime_path,
            self.source_marking_waypoints,
        )
        ...
        self.publish_marking_active(False)
        self.publish_p1_approach_active(True)
        self.publish_p1_direct_a1_active(True)
        self.publish_mission_complete(False)
        ...
        self.get_logger().warn(
            "========== P1 RUNTIME APPROACH BUILT =========="
        )
        self.get_logger().warn(
            f"Mission start       : E={start[0]:.3f}, N={start[1]:.3f}"
        )
        self.get_logger().warn(
            f"APPROACH A1         : E={a1[0]:.3f}, N={a1[1]:.3f}"
        )
        self.get_logger().warn(
            f"A1 distance to P1   : "
            f"{math.hypot(p1[0]-a1[0], p1[1]-a1[1]):.3f}m"
        )
        self.get_logger().warn(
            "Current -> A1       : direct target; no interpolation"
        )
        self.get_logger().warn(
            f"A1 path index       : {self.p1_approach_index + 1}"
        )
        self.get_logger().warn(
            f"A1 -> P1 points     : {len(a1_to_p1)}"
        )
        self.get_logger().warn(
            f"Marking indices     : {marking_indices}"
        )
        self.get_logger().warn(
            f"Runtime path points : {len(self.path_points)}"
        )
        self.get_logger().warn(
            f"Runtime mission ID  : {self.runtime_signature[:12]}"
        )
        self.get_logger().warn(
            "P2-P4 original interpolated trajectory preserved"
        )
    ...
    def mission_enable_callback(self, msg):
        ...
        if enabled and not self.runtime_built:
            self.runtime_build_requested = True
            self.get_logger().warn(
                "P1 RUNTIME APPROACH BUILD REQUESTED"
            )
    ...
    def publish_active_waypoint(self, index):
        x, y = self.path_points[index]
        bearing = self.path_bearing_for_index(index)
        ...
        # Orientation carries the fixed ENU path bearing for this target.
        # The patched current RPP uses this for both the generated P1
        # approach and the original P1-P4 interpolated segments.
        msg.pose.orientation.z = math.sin(0.5 * bearing)
        msg.pose.orientation.w = math.cos(0.5 * bearing)

        self.active_waypoint_pub.publish(msg)

    def path_bearing_for_index(self, index):
        target = self.path_points[index]

        if index == 0:
            start = self.runtime_start_point
        else:
            start = self.path_points[index - 1]
        ...
        if math.hypot(delta_x, delta_y) <= 1.0e-9:
            if index + 1 < len(self.path_points):
                next_point = self.path_points[index + 1]
                delta_x = next_point[0] - target[0]
                delta_y = next_point[1] - target[1]
            else:
                return 0.0
    ...
    def publish_p1_approach_active(self, active):
        msg = Bool()
        msg.data = bool(active)
        self.p1_approach_active_pub.publish(msg)

    def publish_p1_direct_a1_active(self, active):
        msg = Bool()
        msg.data = bool(active)
        self.p1_direct_a1_active_pub.publish(msg)
    ...
    def marking_handoff_distance(self, marking_index):
        if self.marking_numbers[marking_index] == 1:
            return self.p1_marking_handoff_distance
        return self.marking_lookahead_distance
    ...
    def promote_near_marking_target(self):
        ...
        handoff_distance = self.marking_handoff_distance(marking_index)

        if marking_distance <= handoff_distance:
            old_index = self.current_path_index
            self.current_path_index = marking_index
            self.get_logger().warn(
                "MARKING APPROACH ACTIVATED | "
                f"wp={self.marking_numbers[marking_index]} | "
                f"path={old_index + 1}->{marking_index + 1} | "
                f"distance={marking_distance:.3f}m | "
                f"handoff={handoff_distance:.3f}m"
            )

    def select_forward_index_by_path_distance(
        self,
        start_index,
        stop_before_index,
        requested_distance,
    ):
        """Select a non-marking point requested_distance ahead on path."""
        selected_index = start_index
        accumulated_distance = 0.0

        upper_bound = min(stop_before_index, len(self.path_points) - 1)
        for candidate_index in range(start_index + 1, upper_bound + 1):
            ...
            if accumulated_distance >= requested_distance:
                break

        return selected_index, accumulated_distance

    def advance_pass_through_targets(self):
        """Keep targets ahead without skipping the P1 precision corridor."""
        if not self.path_points:
            return

        # Direct current-position -> A1 phase. Switch before A1 becomes
        # a close target so the rover never pivots toward an almost-passed A1.
        if (
            self.p1_approach_index is not None
            and self.current_path_index == self.p1_approach_index
            and self.p1_marking_index is not None
        ):
            current_distance = self.distance_to_index(
                self.current_path_index
            )
            next_distance = self.distance_to_index(
                self.current_path_index + 1
            )
            clearly_passed = (
            ...
            if (
                current_distance <= self.p1_a1_switch_distance
                or clearly_passed
            ):
                selected_index, lookahead = (
                    self.select_forward_index_by_path_distance(
                        self.p1_approach_index,
                        self.p1_marking_index - 1,
                        self.p1_corridor_lookahead_distance,
                    )
                )

                if selected_index <= self.p1_approach_index:
                    selected_index = min(
                        self.p1_approach_index + 1,
                        self.p1_marking_index - 1,
                    )
                ...
                self.get_logger().warn(
                    "A1 HANDOFF TO P1 CORRIDOR | "
                    f"path={old_index + 1}->{selected_index + 1} | "
                    f"a1_distance={current_distance:.3f}m | "
                    f"corridor_lookahead={lookahead:.3f}m"
                )
            return

        # A1 -> P1 precision corridor. Use a short corridor lookahead and
        # never advance directly onto P1. The P1 handoff is owned by
        # promote_near_marking_target() at 0.18 m.
        if (
            self.p1_marking_index is not None
            and self.p1_approach_index is not None
            and self.p1_approach_index
            < self.current_path_index
            < self.p1_marking_index
        ):
            while self.current_path_index < self.p1_marking_index - 1:
        ...
        # Preserve the original P2-P4 behavior.
        while (
    ...
    def control_loop(self):
        if self.runtime_build_requested and not self.runtime_built:
            if not self.source_path or not self.source_marking_waypoints:
                self.publish_p1_approach_active(False)
                self.publish_p1_direct_a1_active(False)
                self.log_waiting("waiting for source mission")
                return
            if self.current_x is None or self.current_y is None:
                self.publish_p1_approach_active(False)
                self.publish_p1_direct_a1_active(False)
                self.log_waiting("waiting for odometry before P1 build")
                return
            if not self.odometry_is_fresh():
                self.publish_p1_approach_active(False)
                self.publish_p1_direct_a1_active(False)
                self.log_waiting("fresh odometry required for P1 build")
                return

            try:
                self.build_runtime_p1_approach()
            except ValueError as error:
                self.publish_p1_approach_active(False)
                self.publish_p1_direct_a1_active(False)
                self.log_rejected(str(error))
                return

        if not self.runtime_built or not self.path_points:
            self.publish_marking_active(False)
            self.publish_p1_approach_active(False)
            self.publish_p1_direct_a1_active(False)
            self.log_waiting(
                "mission loaded; enable mission to build P1 approach"
            )
            return
        ...
        p1_approach_active = (
            self.p1_marking_index is not None
            and self.current_path_index <= self.p1_marking_index
        )
        p1_direct_a1_active = (
            self.p1_approach_index is not None
            and self.current_path_index == self.p1_approach_index
            and not target_is_marking
        )
        self.publish_p1_approach_active(p1_approach_active)
        self.publish_p1_direct_a1_active(p1_direct_a1_active)
        self.publish_active_waypoint(self.current_path_index)

        if (
            not self.a1_direct_log_emitted
            and self.p1_approach_index is not None
            and self.current_path_index == self.p1_approach_index
        ):
            self.a1_direct_log_emitted = True
            self.get_logger().warn(
                "DIRECT CURRENT->A1 TARGET ACTIVE / NO INTERPOLATION"
            )

        if (
            not self.p1_corridor_log_emitted
            and self.p1_approach_index is not None
            and self.current_path_index > self.p1_approach_index
            and self.current_path_index <= self.p1_marking_index
        ):
            self.p1_corridor_log_emitted = True
            self.get_logger().warn(
                "FINAL P1 APPROACH CORRIDOR A1->P1 ACTIVE"
            )
...
def main(args=None):
    ...
    try:
        ...
        node.publish_p1_approach_active(False)
        node.publish_p1_direct_a1_active(False)
        node.destroy_node()
        if rclpy.ok():
            rclpy.shutdown()
