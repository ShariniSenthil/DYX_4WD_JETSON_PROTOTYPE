...
class RPPController(Node):
    """
    Forward-only precision controller for the interpolated marking mission.

    Control policy:
        - pass-through points are tracked using a mission-manager lookahead;
        - marking alignment begins before the final 0.50 m zone;
        - no stationary pivot is allowed inside the final 0.50 m zone;
        - speed is reduced in two stages near a marking point;
        - marking completion is owned by mission_manager;
        - a missed 50 mm capture results in a safe hold, not a circular
          close-range re-approach.

    Motion outputs are limited to:
        - forward path tracking;
        - stationary left/right pivot using a +/-90 degree side vector;
        - stop.

    No reverse target vector is generated.
    """
    ...
    def __init__(self):
        ...
        self.declare_parameter("turn_speed_mps", 0.05)
        self.declare_parameter("marking_approach_speed_mps", 0.06)
        self.declare_parameter("marking_near_speed_mps", 0.05)
        self.declare_parameter("marking_near_distance_m", 0.15)
        self.declare_parameter("pivot_vector_speed_mps", 0.2)
        self.declare_parameter("slow_heading_error_deg", 20.0)
        self.declare_parameter("pivot_enter_angle_deg", 4.0)
        self.declare_parameter("pivot_exit_angle_deg", 2.0)
        self.declare_parameter("marking_alignment_pivot_deg", 15.0)
        self.declare_parameter("pivot_hold_sec", 0.75)
        self.declare_parameter("marking_alignment_distance_m", 1.10)
        self.declare_parameter("marking_no_pivot_distance_m", 1.00)
        self.declare_parameter("final_bearing_lock_distance_m", 0.25)
        self.declare_parameter("final_bearing_lock_error_deg", 10.0)
        self.declare_parameter("final_alignment_abort_distance_m", 0.12)
        ...
        # Straight bearing used while approaching a marking waypoint.
        # This does not activate capture-missed monitoring.
        ...
        self.get_logger().warn(
            "===== PRECISION LOOKAHEAD RPP STARTED ====="
        )
        self.get_logger().warn(
            "Motion policy: forward, stationary pivot, stop; no reverse"
        )
        self.get_logger().warn(
        ...
        self.get_logger().warn(
            "Marking speeds: "
            f"{self.marking_approach_speed:.3f} m/s below "
        ...
        self.get_logger().warn(
            f"Pivot enter/exit: {self.pivot_enter_angle_deg:.0f}/"
    ...
    def reset_motion_state(self):
        ...
        # Clear all per-attempt terminal capture state.
        # This is required whenever mission is disabled, E-stop is
        # asserted, or a new marking attempt begins.
        self.final_bearing_enu = None
        self.marking_approach_bearing_enu = None
        self.closest_distance = math.inf
        self.marking_missed = False
    ...
    def nearest_marking_segment_bearing(self, x, y):
        """
        Return the fixed ENU bearing of the original marking segment
        nearest to an interpolated pass-through target.
        ...
        for start, end in zip(
            ...
            projected_east = (
                start_east + projection * segment_east
            )
            projected_north = (
                start_north + projection * segment_north
            )
    ...
    def waypoint_callback(self, msg):
        ...
        # Preserve the fixed segment bearing before update_target_type()
        # clears it when the next target is an original marking waypoint.
        ...
        if changed:
            ...
            # When a straight pass-through segment hands over to its
            # destination marking waypoint, continue on exactly the same
            # fixed segment bearing. Only the speed decreases.
            #
            # For WP1 there is no preceding pass-through bearing, so the
            # existing direct-bearing approach remains unchanged.
            if (
                self.target_is_marking
                and incoming_segment_bearing_enu is not None
                and math.isfinite(incoming_segment_bearing_enu)
            ):
    ...
    def marking_active_callback(self, msg):
        ...
        if active:
            ...
            # /marking_active and /active_waypoint are different ROS topics,
            # so their delivery order is not guaranteed. Hold zero after the
            # dwell is released until a new target has been received. This
            # prevents the completed marking point from being evaluated by
            # the miss detector during the handoff.
            if self.target_is_marking:
    ...
    def publish_velocity_ned(
        ...
        # vector.z is reserved for ENU yaw rate:
    ...
    def publish_pivot(self, distance, heading_error):
        """
        Send the fixed target bearing through PX4 velocity OFFBOARD.

        PX4's custom differential velocity controller converts this
        bearing into a zero-longitudinal-speed spot turn. Unlike the old
        current-yaw +/- 90 degree vector, this bearing does not move as
        the rover rotates.
        """
        target_yaw_enu = math.atan2(
            math.sin(self.current_yaw_enu + heading_error),
            math.cos(self.current_yaw_enu + heading_error),
        )

        # Send only a microscopic vector magnitude. PX4 uses the
        # vector direction as the requested yaw while commanded
        # translational speed remains effectively zero.
        heading_vector_speed = self.pivot_vector_speed

        velocity_east = (
            heading_vector_speed * math.cos(target_yaw_enu)
        )
        velocity_north = (
            heading_vector_speed * math.sin(target_yaw_enu)
        )

        pivot_side = "LEFT" if heading_error >= 0.0 else "RIGHT"

        # yaw_rate remains zero. PX4 receives a velocity bearing and
        # performs the differential spot turn internally.
    ...
    def bounded_marking_guidance_bearing(
        ...
        """
        Keep the established marking approach line while permitting
        a small correction toward the live marking coordinate.

        The correction is bounded by final_bearing_lock_error_deg,
        preventing unrestricted direct-to-point steering near the
        waypoint.
        """
    ...
    def control_loop(self):
        ...
        if self.pivot_active:
            if pivot_forbidden_near_marking:
                ...
                # The current direct bearing points from the rover's
                # present position to the marking coordinate. Preserve
                # it as a straight approach line before pivoting is
                # prohibited.
                if (
                    ...
                    self.get_logger().warn(
                        ...
                        f"bearing="
                        f"{math.degrees(self.marking_approach_bearing_enu):.1f}deg ENU"
                    )
                else:
                    self.get_logger().warn(
        ...
        if self.target_is_marking:
            # Marking targets farther than the alignment zone continue
            # along the established incoming segment guidance.
            #
            # Direct-to-marking pivot is permitted only inside:
            # marking_alignment_distance > distance >
            # marking_no_pivot_distance.
            in_alignment_zone = (
                self.marking_no_pivot_distance
                < distance
                <= self.marking_alignment_distance
            ...
            # Generated path points use the fixed original segment
            # bearing. For a large corner error, align nearly in place
            # before beginning straight-line translation.
            if (
                pivot_heading_error_deg
        ...
        # The rover may enter the no-pivot zone while already
        # reasonably aligned, meaning pivot_active was never set.
        # Preserve the direct target bearing at that moment so the
        # remaining approach follows one fixed straight line.
        if (
            self.target_is_marking
            and distance <= self.marking_no_pivot_distance
            ...
            approach_bearing_deg = math.degrees(
                self.marking_approach_bearing_enu
            )
            ...
            self.get_logger().warn(
                ...
                f"bearing={approach_bearing_deg:.1f}deg ENU"
            )
            ...
            if distance <= self.final_bearing_lock_distance:
                ...
                if (
                    ...
                    self.get_logger().warn(
                        ...
                        f"bearing="
                        f"{math.degrees(self.final_bearing_enu):.1f}deg ENU"
                    )
                else:
        ...
        if (
            ...
            # Real terminal lock. Capture monitoring is active.
            ...
            # Straight marking approach. Capture monitoring is not
            # active until final_bearing_enu is assigned.
        ...
        if self.target_is_marking:
            if distance <= self.marking_near_distance:
                ...
                # Smooth cubic deceleration from forward_speed at the
                # alignment boundary to marking_near_speed at the near
                # boundary. The cubic smoothstep has no sudden slope
                # change at either end of the deceleration zone.
                ramp_span = (
                    self.marking_alignment_distance
                    - self.marking_near_distance
                )
            ...
            # Align to the fixed segment direction slowly, then increase
            # speed smoothly. Full speed is permitted only after the
            # rover is within 3 degrees of the straight-line bearing.
        ...
        self.publish_velocity_ned(velocity_north, velocity_east)
        self.log_control(
            status,
            distance,
            heading_error,
