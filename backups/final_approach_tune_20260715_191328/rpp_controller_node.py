...
class RPPController(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("pivot_vector_speed_mps", 0.20)
        self.declare_parameter("slow_heading_error_deg", 20.0)
        self.declare_parameter("pivot_enter_angle_deg", 45.0)
        self.declare_parameter("pivot_exit_angle_deg", 5.0)
        self.declare_parameter("marking_alignment_pivot_deg", 15.0)
        self.declare_parameter("pivot_hold_sec", 0.75)
        self.declare_parameter("marking_alignment_distance_m", 0.75)
        self.declare_parameter("marking_no_pivot_distance_m", 0.50)
        self.declare_parameter("final_bearing_lock_distance_m", 0.25)
        self.declare_parameter("final_bearing_lock_error_deg", 5.0)
    ...
    def publish_pivot(self, distance, heading_error):
        pivot_yaw_enu = self.normalize_angle(
            self.current_yaw_enu
            + self.pivot_direction * (math.pi / 2.0)
        )
        velocity_east = self.pivot_vector_speed * math.cos(pivot_yaw_enu)
        velocity_north = self.pivot_vector_speed * math.sin(pivot_yaw_enu)
        pivot_side = "LEFT" if self.pivot_direction > 0.0 else "RIGHT"
    ...
    def control_loop(self):
        ...
        if self.pivot_active:
            ...
            if direct_heading_error_deg <= self.pivot_exit_angle_deg:
                self.pivot_active = False
                self.pivot_direction = 0.0
            ...
            self.publish_pivot(distance, direct_heading_error)
            return
        ...
        if self.target_is_marking:
            ...
            if (
                distance > self.marking_no_pivot_distance
                and direct_heading_error_deg
                >= (
                    self.marking_alignment_pivot_deg
                    if in_alignment_zone
                    else self.pivot_enter_angle_deg
                )
            ):
            ...
            if (
                distance <= self.pass_through_no_pivot_distance
                and direct_heading_error_deg
                >= self.pivot_enter_angle_deg
            ):
                self.publish_stop()
                self.log_control(
                    "LOOKAHEAD TARGET TOO CLOSE / WAITING FOR ADVANCE",
                    distance,
                    direct_heading_error,
                    0.0,
                    0.0,
                ...
                return
            if direct_heading_error_deg >= self.pivot_enter_angle_deg:
                self.start_pivot(direct_heading_error)
                self.publish_pivot(distance, direct_heading_error)
                return
        ...
        if (
            ...
            # Keep using the live bearing between 0.50 m and the terminal
            # lock distance. This allows forward-only cross-track correction
            # without a stationary pivot near the marking coordinate.
            if distance <= self.final_bearing_lock_distance:
                if (
                    ...
                    self.alignment_hold_active = False
                    self.final_bearing_enu = direct_bearing_enu
                    ...
                    self.get_logger().warn(
                        ...
                        f"heading_error={direct_heading_error_deg:.1f}deg | "
                        f"bearing="
                        f"{math.degrees(direct_bearing_enu):.1f}deg ENU"
                    )
                else:
        ...
        guidance_bearing_enu = (
            self.final_bearing_enu
            if (
                self.target_is_marking
                and self.final_bearing_enu is not None
            )
            else direct_bearing_enu
        )
        ...
        if self.target_is_marking:
            ...
            elif distance <= self.marking_no_pivot_distance:
                speed = self.marking_approach_speed
                status = (
                    "MARKING TERMINAL STRAIGHT / FORWARD"
                    if self.final_bearing_enu is not None
                    else "MARKING FINAL GUIDANCE / FORWARD"
                )
            elif distance <= self.marking_alignment_distance:
                speed = self.marking_approach_speed
                status = "MARKING ALIGNMENT APPROACH / FORWARD"
            ...
            speed = self.turn_speed
            status = "LOOKAHEAD / HEADING CORRECTION"
        else:
            speed = self.forward_speed
            status = "LOOKAHEAD PASS-THROUGH / FORWARD"
